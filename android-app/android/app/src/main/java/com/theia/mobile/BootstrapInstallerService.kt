...
class BootstrapInstallerService(private val context: Context) {
    ...
    private fun writeWrapperScripts() {
        ...
        val compatWrapper = buildString {
            ...
            appendLine("HOST_ENV=\"${termuxPrefix.absolutePath}/bin/env\"")
            ...
            appendLine("TERMUX_UID=\$(id -u 2>/dev/null || true)")
            appendLine("echo \"DevPocket: host Termux compatibility mode (launcher=\${PROOT_BIN} host-prefix=\${HOST_PREFIX} host-env=\${HOST_ENV} legacy-prefix=\${LEGACY_PREFIX})\" >&2")
            appendLine("if [ ! -x \"\${HOST_ENV}\" ]; then")
            appendLine("  echo \"DevPocket error: missing host env binary at \${HOST_ENV}\" >&2")
            appendLine("  exit 127")
            appendLine("fi")
            ...
            appendLine("  -w / \\")
            appendLine("  \"\${HOST_ENV}\" -i \\")
            appendLine("    PREFIX=\"\${LEGACY_PREFIX}\" HOME=\"\${LEGACY_HOME}\" TMPDIR=\"\${LEGACY_TMP}\" \\")
            appendLine("    PATH=\"\${HOST_PREFIX}/bin:\${LEGACY_PREFIX}/bin:/system/bin\" LD_LIBRARY_PATH=\"\${HOST_PREFIX}/lib:\${LEGACY_PREFIX}/lib\" \\")
