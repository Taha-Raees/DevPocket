...
class BootstrapInstallerService(private val context: Context) {
    ...
    private fun writeWrapperScripts() {
        ...
        val compatWrapper = buildString {
            ...
            appendLine("TERMUX_UID=\$(id -u 2>/dev/null || true)")
            appendLine("echo \"DevPocket: host Termux compatibility mode (launcher=\${PROOT_BIN})\" >&2")
            appendLine("if [ ! -x \"\${LEGACY_PREFIX}/bin/env\" ]; then")
            appendLine("  echo \"DevPocket error: missing legacy env binary at \${LEGACY_PREFIX}/bin/env\" >&2")
            appendLine("  exit 127")
            appendLine("fi")
