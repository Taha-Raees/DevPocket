...
class BootstrapInstallerService(private val context: Context) {
    ...
    private fun writeWrapperScripts() {
        ...
        val compatWrapper = buildString {
            ...
            appendLine("TERMUX_UID=\$(id -u 2>/dev/null || true)")
            appendLine("echo \"DevPocket: host Termux compatibility mode (launcher=\${PROOT_BIN} host-prefix=\${HOST_PREFIX} legacy-prefix=\${LEGACY_PREFIX})\" >&2")
            appendLine("if [ ! -x \"\${HOST_PREFIX}/bin/env\" ]; then")
            appendLine("  echo \"DevPocket error: missing host env binary at \${HOST_PREFIX}/bin/env\" >&2")
            appendLine("  exit 127")
            appendLine("fi")
            appendLine("if [ ! -x \"\${HOST_PREFIX}/bin/bash\" ]; then")
            appendLine("  echo \"DevPocket error: missing host bash binary at \${HOST_PREFIX}/bin/bash\" >&2")
            appendLine("  exit 127")
            appendLine("fi")
            ...
            appendLine("  -w / \\")
            appendLine("  \"\${LEGACY_PREFIX}/bin/env\" -i \\")
